...
const Part3 = ({ row }) => {
    ...
    const {
        ...
        errorQues,
        data: getParaPart3
    } = useQuery("getParaPart3", () => PartSV.getParagraphWitPart({ idPart: 3 }))

    const [paragraphs, setParagraphs] = useState([])

    useEffect(() => {
        if (globalState.connect) {
            if (!isLoadingPara && !isErrorPara && getParaPart3 !== undefined) {
                if (getParaPart3.data !== null)
                    setParagraphs(getParaPart3.data.listPara)
            }
        }
    }, [isLoadingPara, globalState.connect, isErrorPara, getParaPart3]);
    console.log(getParaPart3)
        ...
        <Row align={"middle"} gutter={[16, 16]} style={{ width: "100%" }}>
            ...
            >
                <Tooltip title={"Số đoạn văn được chọn phải là 13"} color="green" >
                    <p style={{ position: "absolute", color: row.selectedRowKeys.length === 13 ? 'green' : 'red' }}>
